...
Project {
    ...
    ConanfileProbe {
        ...
        additionalArguments: {
            ...
            // On Windows, Conan package resolution must match the Qbs build variant.
            // Otherwise Conan may look for an invalid hybrid of Debug + Release runtime.
            if (qbs.targetOS.contains("windows")) {
                if (qbs.buildVariant === "debug") {
                    args.push("-s=build_type=Debug");
                    args.push("-s=compiler.runtime_type=Debug");
                } else if (qbs.buildVariant === "profiling") {
                    args.push("-s=build_type=RelWithDebInfo");
                    args.push("-s=compiler.runtime_type=Release");
                } else {
                    args.push("-s=build_type=Release");
                    args.push("-s=compiler.runtime_type=Release");
                }
            }
