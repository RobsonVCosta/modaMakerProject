...
//---------------------------------------------------------------------------------------------------------------------
auto FormatRemainingTime(const QDateTime &expiration) -> QString
{
    ...
    qint64 secs = QDateTime::currentDateTimeUtc().secsTo(expiration);
    if (secs <= 0)
    {
        return QObject::tr("Expired");
    }

    const qint64 days = secs / (24 * 60 * 60);
    secs %= (24 * 60 * 60);
    const qint64 hours = secs / (60 * 60);
    secs %= (60 * 60);
    const qint64 minutes = secs / 60;

    return QObject::tr("%1 day(s), %2 hour(s), %3 minute(s)").arg(days).arg(hours).arg(minutes);
}

class ProfileEasterEggFilter final : public QObject
{
public:
    explicit ProfileEasterEggFilter(const QList<QWidget *> &hiddenWidgets, QObject *parent = nullptr)
      : QObject(parent),
        m_hiddenWidgets(hiddenWidgets)
    {
    }
...
protected:
    auto eventFilter(QObject *watched, QEvent *event) -> bool override
    {
        ...
        constexpr int sequence[] = {Qt::Key_Up, Qt::Key_Up, Qt::Key_Down,
                                    Qt::Key_Down, Qt::Key_Left, Qt::Key_Right};

        if (keyEvent->key() == sequence[m_index])
        {
            ++m_index;
            if (m_index == static_cast<int>(std::size(sequence)))
            {
                for (QWidget *widget : m_hiddenWidgets)
                {
                    if (widget != nullptr)
                    {
        ...
        {
            m_index = (keyEvent->key() == sequence[0]) ? 1 : 0;
        }
    ...
    m_gradation(new QTimer(this))
{
    ...
    InitDocksContain();
    CreateMenus();
    ui->actionReportBug->setVisible(false);
    ToolBarDraws();
    ToolBarPenOptions();
    ToolBarStages();
    ToolBarDrawTools();
    ...
    accountMenu->setStyleSheet(QStringLiteral("QMenu { min-width: 160px; }"));
    accountMenu->addAction(tr("Perfil"), this, &MainWindow::ShowAccountProfile);
    accountMenu->addAction(tr("Logout"), this, &MainWindow::LogoutAccount);

    m_accountButton = new QToolButton(this);
    m_accountButton->setToolButtonStyle(Qt::ToolButtonIconOnly);
    m_accountButton->setPopupMode(QToolButton::InstantPopup);
    m_accountButton->setMenu(accountMenu);
    m_accountButton->setToolTip(tr("Logged in"));
    m_accountButton->setIcon(QIcon(QStringLiteral(":/icon/32x32/userIcon.png")));
    m_accountButton->setIconSize(QSize(32, 32));

    if (menuBar() != nullptr)
    {
        menuBar()->setCornerWidget(m_accountButton, Qt::TopRightCorner);
    }
...
//---------------------------------------------------------------------------------------------------------------------
void MainWindow::ShowAccountProfile()
{
    AuthenticatedUser user;
    QString resultText;
    QString errorMessage;
    if (!DialogLogin::FetchCurrentUser(user, resultText, errorMessage))
    {
        QMessageBox::warning(this,
                             tr("Perfil"),
                             errorMessage.isEmpty() ? tr("Unable to load account information.") : errorMessage);
        return;
    }

    QDialog dialog(this);
    dialog.setWindowTitle(tr("Perfil"));
    dialog.setModal(true);

    auto *nameLabel = new QLabel(user.name, &dialog);
    nameLabel->setTextInteractionFlags(Qt::NoTextInteraction);
    nameLabel->setWordWrap(true);

    auto *emailLabel = new QLabel(user.email, &dialog);
    emailLabel->setTextInteractionFlags(Qt::NoTextInteraction);
    emailLabel->setWordWrap(true);

    auto *expirationLabel = new QLabel(FormatRemainingTime(DialogLogin::AccessTokenExpiration()), &dialog);
    expirationLabel->setTextInteractionFlags(Qt::NoTextInteraction);
    expirationLabel->setWordWrap(true);

    auto *refreshExpirationLabel =
        new QLabel(FormatRemainingTime(DialogLogin::RefreshTokenExpiration()), &dialog);
    refreshExpirationLabel->setTextInteractionFlags(Qt::NoTextInteraction);
    refreshExpirationLabel->setWordWrap(true);

    auto *formLayout = new QFormLayout();
    formLayout->addRow(tr("Name:"), nameLabel);
    formLayout->addRow(tr("Email:"), emailLabel);
    formLayout->addRow(tr("Access token expires in:"), expirationLabel);
    formLayout->addRow(tr("Refresh token expires in:"), refreshExpirationLabel);

    QWidget *accessLabel = formLayout->labelForField(expirationLabel);
    QWidget *refreshLabel = formLayout->labelForField(refreshExpirationLabel);
    const QList<QWidget *> hiddenProfileWidgets{accessLabel, refreshLabel, expirationLabel, refreshExpirationLabel};
    for (QWidget *widget : hiddenProfileWidgets)
    {
        if (widget != nullptr)
        {
    ...
    {
        layout->addWidget(new QLabel(resultText, &dialog));
    }
    layout->addWidget(buttonBox);

    dialog.setFocusPolicy(Qt::StrongFocus);
    dialog.installEventFilter(
        new ProfileEasterEggFilter({accessLabel, refreshLabel, expirationLabel, refreshExpirationLabel}, &dialog));
    dialog.setFocus();
...
void MainWindow::LogoutAccount()
{
    ...
    QString resultText;
    QString errorMessage;
    if (!DialogLogin::LogoutCurrentUser(resultText, errorMessage))
    {
        QMessageBox::warning(
            this, tr("Logout"), errorMessage.isEmpty() ? tr("Unable to log out right now.") : errorMessage);
        return;
    }

    QMessageBox::information(this, tr("Logout"), resultText.isEmpty() ? tr("Logout successful.") : resultText);

    if (!DialogLogin::EnsureAuthenticated(this))
    {
...
void MainWindow::CreateActions()
{
    ...
    connect(ui->actionShop, &QAction::triggered, this,
            []()
            {
                ...
                QDesktopServices::openUrl(QUrl(QStringLiteral("https://www.yourdomain.com/")));
            });
