...
inline void noisyFailureMsgHandler(QtMsgType type, const QMessageLogContext &context, const QString &msg)
{
    ...
    if (isGuiThread)
    {
        if ((type == QtWarningMsg || type == QtCriticalMsg || type == QtFatalMsg) && VApplication::IsGUIMode())
        {
            ...
            {
                const QString cleanMessage = VAbstractValApplication::ClearMessage(logMsg);
                const QString normalizedMessage = cleanMessage.toLower();
                if (normalizedMessage.contains(QStringLiteral("stream 1 finished with error")) ||
                    normalizedMessage.contains(QStringLiteral("o servidor requer autenticação")) ||
                    normalizedMessage.contains(QStringLiteral("o servidor requer autenticacao")) ||
                    normalizedMessage.contains(QStringLiteral("server requires authentication")))
                {
                    return;
                }
                ...
                messageBox.setText(cleanMessage);
