...
public:
        ...
        tolfit(p.tolfit)
    {
        ...
        for (const auto *item : p.controllist)
        {
            controllist.push_back(item != nullptr ? new DRW_Coord(*item) : nullptr);
        }

        for (const auto *item : p.fitlist)
        {
            fitlist.push_back(item != nullptr ? new DRW_Coord(*item) : nullptr);
        }
    }
