...
void VToolTrueDarts::SaveDialog(QDomElement &domElement)
{
    ...
    doc->SetAttributeOrRemoveIf<QString>(domElement, AttrName1, dialogTool->GetFirstNewDartPointName(),
                                         [](const QString &name) noexcept { return name.isEmpty(); });
    doc->SetAttributeOrRemoveIf<QString>(domElement, AttrName2, dialogTool->GetSecondNewDartPointName(),
                                         [](const QString &name) noexcept { return name.isEmpty(); });
